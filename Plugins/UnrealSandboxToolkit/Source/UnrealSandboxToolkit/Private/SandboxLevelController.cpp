...
// Статические переменные для хранения карты объектов и экземпляра контроллера уровня
...
// Конструктор для ASandboxLevelController
ASandboxLevelController::ASandboxLevelController() {
    MapName = TEXT("World 0"); // Установка имени карты
}
...
void ASandboxLevelController::BeginPlay() {
    ...
    StaticSelf = this; // Установка статического экземпляра
}

// Вызывается, когда игра заканчивается
void ASandboxLevelController::EndPlay(const EEndPlayReason::Type EndPlayReason) {
    Super::EndPlay(EndPlayReason); // Вызов функции EndPlay родительского класса
    StaticSelf = nullptr; // Сброс статического экземпляра
}
...
// Сохранение подготовленных объектов в JSON
...
// Создание дескриптора объекта песочницы
...
// Подготовка объектов для сохранения
...
// Расширение для сохранения уровня в JSON
...
// Создание подготовленного объекта
...
// Создание подготовленных объектов
...
// Расширение для загрузки уровня из JSON
void ASandboxLevelController::LoadLevelJsonExt(TSharedPtr<FJsonObject> JsonParsed) {
    // Реализация расширения
}

// Получение объекта песочницы по идентификатору класса
...
// Получение объекта песочницы по умолчанию
...
// Получение экземпляра контроллера уровня
