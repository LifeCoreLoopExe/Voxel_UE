// Авторские права blackw 2015-2020

#pragma once // Директива препроцессора для предотвращения множественного включения заголовочного файла

#include "Engine.h" // Подключение основного движка Unreal Engine
#include "Components/ActorComponent.h" // Подключение базового класса компонента актёра
#include "ContainerComponent.generated.h" // Генерация метаданных для редактора Unreal Engine

class ASandboxObject; // Предварительное объявление класса ASandboxObject

USTRUCT() // Макрос для объявления структуры, доступной в редакторе Unreal Engine
struct FContainerStack { // Объявление структуры FContainerStack
	GENERATED_USTRUCT_BODY() // Макрос для генерации метаданных структуры

	UPROPERTY(EditAnywhere) // Свойство, доступное для редактирования в редакторе
	int32 Amount; // Количество объектов в стеке

	UPROPERTY(EditAnywhere) // Свойство, доступное для редактирования в редакторе
	uint64 SandboxClassId; // Идентификатор класса объекта песочницы

	FContainerStack() { // Конструктор по умолчанию
		Clear(); // Очистка значений
	}

	FContainerStack(int32 Amount_, uint64 SandboxClassId_) : Amount(Amount_), SandboxClassId(SandboxClassId_) { } // Конструктор с параметрами

	void Clear() { // Метод для очистки значений
		Amount = 0;
		SandboxClassId = 0;
	}

	bool IsEmpty() const { // Метод для проверки, пуст ли стек
		return Amount == 0;
	}

	const ASandboxObject* GetObject() const; // Метод для получения объекта песочницы (реализация не предоставлена)
};

UCLASS( ClassGroup=(Custom), meta=(BlueprintSpawnableComponent) ) // Макрос для объявления класса, доступного в редакторе и Blueprint
class UNREALSANDBOXTOOLKIT_API UContainerComponent : public UActorComponent { // Объявление класса UContainerComponent, наследуемого от UActorComponent
	GENERATED_BODY() // Макрос для генерации метаданных класса

public:
	UPROPERTY(ReplicatedUsing = OnRep_Content, EditAnywhere, Category = "Sandbox") // Свойство, реплицируемое и доступное для редактирования в редакторе
	int32 MaxCapacity = 13; // Максимальная ёмкость контейнера

	UPROPERTY(ReplicatedUsing = OnRep_Content, EditAnywhere, Category = "Sandbox") // Свойство, реплицируемое и доступное для редактирования в редакторе
	TArray<FContainerStack> Content; // Массив стеков контейнера

public:
	UContainerComponent(); // Конструктор по умолчанию

	virtual void BeginPlay() override; // Метод, вызываемый при начале игры

	virtual void TickComponent( float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction ) override; // Метод, вызываемый каждый тик

	UFUNCTION() // Макрос для объявления функции, доступной в редакторе
	void OnRep_Content(); // Метод, вызываемый при репликации содержимого

	const TArray<FContainerStack>& GetContent(); // Метод для получения содержимого контейнера

	bool SetStackDirectly(const FContainerStack& Stack, const int SlotId); // Метод для установки стека напрямую

	bool AddObject(ASandboxObject* Obj); // Метод для добавления объекта в контейнер

	FContainerStack* GetSlot(const int Slot); // Метод для получения стека по индексу

	const FContainerStack* GetSlot(const int Slot) const; // Метод для получения стека по индексу (константная версия)

	void ChangeAmount(int Slot, int Num); // Метод для изменения количества объектов в стеке

    bool DecreaseObjectsInContainer(int slot, int num); // Метод для уменьшения количества объектов в контейнере

	bool IsEmpty() const; // Метод для проверки, пуст ли контейнер

	bool IsSlotEmpty(int SlotId) const; // Метод для проверки, пуст ли слот

	void CopyTo(UContainerComponent* Target); // Метод для копирования содержимого в другой контейнер

	TArray<uint64> GetAllObjects() const; // Метод для получения всех объектов в контейнере

	bool SlotTransfer(int32 SlotDropId, int32 SlotTargetId, AActor* SourceActor, UContainerComponent* SourceContainer, bool bOnlyOne = false); // Метод для перемещения объектов между слотами

	bool IsUpdated(); // Метод для проверки, был ли контейнер обновлён

	void ResetUpdatedFlag(); // Метод для сброса флага обновления

	const TMap<uint64, uint32>& GetStats() const; // Метод для получения статистики контейнера

private:
	TMap<uint64, uint32> InventoryStats; // Статистика инвентаря

	void MakeStats(); // Метод для создания статистики

	bool bUpdated = false; // Флаг обновления
};
